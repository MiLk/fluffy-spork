defmodule FluffySpork.Api.Webhook do
  ...
  defp handle_event(:issues, %{"action" => "closed"}) do {204, ""} end
  ...
  defp handle_event(:pull_request, %{"action" => "closed"}) do {204, ""} end
  ...
  defp create_card(type, repository, issue, labels) do
    ...
    unless FluffySpork.Github.Project.has_card(project_server, {repo_owner, repo_name, number}) do
      project_server |> FluffySpork.Github.Project.create_card(destination, issue_id, type)
    end
